...
4. **Store dataset identifiers** (DOI, handle, native id) plus the catalog `uid`. Do not write dataset YAML into this repository.
...
Nested `software` / `endpoints` are JSON **strings** in DuckDB ([ai-consumers.md](ai-consumers.md)).
