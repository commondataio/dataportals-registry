...
try:
    ...
    logger.error("zstandard package not found. Install it with: pip install zstandard")
    sys.exit(1)
    ...
    for item in items:
        for key, value in item.items():
            if isinstance(value, dict):
                ...
                result.append(
                    f"{indent_str}- **{key}**: {', '.join(str(v) for v in value)}"
                )
            else:
...
def format_coverage(coverage: List[Dict]) -> str:
    ...
    for item in coverage:
        ...
        if country:
            parts.append(
                f"Country: {country.get('name', 'N/A')} ({country.get('id', 'N/A')})"
            )
        if macroregion:
...
def format_rights(rights: Dict) -> str:
    ...
    if rights.get("license_url"):
        result.append(
            f"**License URL**: [{rights['license_url']}]({rights['license_url']})"
        )
    if rights.get("privacy_policy_url"):
        result.append(
            f"**Privacy Policy**: [{rights['privacy_policy_url']}]({rights['privacy_policy_url']})"
        )
    if rights.get("tos_url"):
        result.append(
            f"**Terms of Service**: [{rights['tos_url']}]({rights['tos_url']})"
        )
...
def convert_jsonl_to_markdown(input_file: str, output_dir: str) -> None:
    ...
    with open(input_file, "rb") as f:
        ...
        with dctx.stream_reader(f) as reader:
            ...
            total = len(lines)
            logger.info("Processing %d records...", total)
            ...
            for idx, line in enumerate(lines, 1):
                try:
                    ...
                    if idx % 100 == 0 or idx == total:
                        logger.info(
                            "Progress: %d/%d (%d%%)", idx, total, idx * 100 // total
                        )
            ...
            logger.info("Successfully converted: %d records", success_count)
            logger.info("Errors: %d records", error_count)
