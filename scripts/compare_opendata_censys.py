#!/usr/bin/env python
"""
Compare Censys opendata (www.opendata*) domains against the registry and identify missing ones.
Output: dev/data/opendata_for_review.txt
"""
import json
import os
import re
...
CENSYS_PATH = os.path.join(_REPO_ROOT, "dev", "data", "censis_opendata_list.json")
FULL_JSONL_PATH = os.path.join(_REPO_ROOT, "data", "datasets", "full.jsonl")
OUTPUT_PATH = os.path.join(_REPO_ROOT, "dev", "data", "opendata_for_review.txt")
...
def main():
    ...
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        f.write("# OpenData (www.opendata*) domains from Censys missing in the registry\n")
        f.write(f"# Total: {len(missing)} domains\n")
        f.write("# Source: dev/data/censis_opendata_list.json\n")
        f.write("# Excluded: IP addresses\n")
        f.write("\n")
        for domain in missing:
            f.write(f"{domain}\n")

    print(f"Censys opendata hosts (total): {len(censys_hosts)}")
    print(f"Censys opendata hostnames (excl. IPs): {len([h for h in censys_hosts if not is_ip_address(h)])}")
