...
class LocationBase(BaseModel):
    ...
    subregion: Optional[SubRegion] = Field(
        None, examples=[{"id": "US-TX", "name": "Texas"}]
    )
    macroregion: Optional[MacroRegion] = Field(
        None, examples=[{"id": "145", "name": "Western Asia"}]
    )
...
class Endpoint(BaseModel):
    ...
    url: str = Field(
        ..., examples=["https://catalog.data.gov/api/3action/package_search"]
    )
...
class Rights:
    ...
    rights_type: str = Field(
        ..., examples=["granular", "global", "unknown", "inapplicable"]
    )
