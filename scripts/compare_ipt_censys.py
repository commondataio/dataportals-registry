#!/usr/bin/env python
"""
Compare Censys IPT (Integrated Publishing Toolkit) domains against the registry
and identify missing ones.
Output: dev/data/ipt_for_review.txt
"""
import json
import os
import re
...
CENSYS_PATH = os.path.join(_REPO_ROOT, "dev", "data", "censys_ipt_list.json")
FULL_JSONL_PATH = os.path.join(_REPO_ROOT, "data", "datasets", "full.jsonl")
OUTPUT_PATH = os.path.join(_REPO_ROOT, "dev", "data", "ipt_for_review.txt")
...
def main():
    ...
    with open(OUTPUT_PATH, "w", encoding="utf-8") as f:
        f.write("# IPT (Integrated Publishing Toolkit) domains from Censys missing in the registry\n")
        f.write(f"# Total: {len(missing)} domains\n")
        f.write("# Source: dev/data/censys_ipt_list.json\n")
        f.write("# Excluded: IP addresses\n")
        f.write("\n")
        for domain in missing:
            f.write(f"{domain}\n")

    print(f"Censys IPT hosts (total): {len(censys_hosts)}")
    print(f"Censys IPT hostnames (excl. IPs): {len([h for h in censys_hosts if not is_ip_address(h)])}")
