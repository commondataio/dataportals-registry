...
- Discovery guides now include fingerprints for high-volume regional and hosted platforms (Wagmap, EWMAPA, GeoMapFish, Tianditu, Masterportal, Liferay, oPortal, Digital Commons, InstDB, WEKO3, and related IDs), plus metadata registries beyond FAIR Data Point. Pipeline docs cover `apidetect.py`, catalog URL liveness, and GitHub Pages deploy. `llms.txt` indexes `SECURITY.md` and `CODE_OF_CONDUCT.md`.
